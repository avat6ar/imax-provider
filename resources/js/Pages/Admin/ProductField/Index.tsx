...
import { Field, Product } from "@/types/dashboard";
...
export const columns: ColumnDef<Field>[] = [
  ...
  },
  {
    ...
    header: "Title Ar",
    cell: ({ row }) => (
      <div className="capitalize">{row.getValue("title_ar")}</div>
    ),
  },
  {
    ...
    header: "Title Ru",
    cell: ({ row }) => (
      <div className="capitalize">{row.getValue("title_ru")}</div>
    ),
  },
  {
    accessorKey: "key",
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="flex gap-4 items-center">
        <Link
          href={route("admin.product-fields.edit", row.getValue("id"))}
          className="cursor-pointer"
        >
          <LucideEdit3 className="text-blue-600 size-5" />
        </Link>
        <Link
          href={route("admin.product-fields.destroy", row.getValue("id"))}
          method="delete"
          as="button"
          className="cursor-pointer"
        >
...
const Index = ({ fields }: { fields: Field[] }) => {
    ...
    <Authenticated>
      ...
      <div className="w-full rounded-xl bg-white py-6 px-3 space-y-3">
        ...
        <div className="flex items-center justify-between">
          ...
          <Link
            href={route("admin.product-fields.create")}
